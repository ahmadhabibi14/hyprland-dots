...
get_match()
{
  selection=$(echo -e "$1" | rofi -dmenu -p "$2" -config ~/.config/waybar/scripts/rofi/networkmenu.rasi)
  [[ -z "$selection" ]] && exit 1
...
if [ "$selected_ssid" = "$create_option" ]; then
  manual_ssid=$(echo "Enter the SSID of the network." | rofi -dmenu -p "SSID" -config ~/.config/waybar/scripts/rofi/networkmenu.rasi)
  ...
  if [[ -n "$matches00" ]];then #If a match exists to the selected networks 
    ...
    #if no matches
    manual_password=$(echo "Enter the Password of the network (or leave blank)." | rofi -dmenu -p "Password" -config ~/.config/waybar/scripts/rofi/networkmenu.rasi)
...
#An existing connection was selected
  ...
  if [[ -n "$matches" ]];then #If a match exists to the selected networks 
    ...
    if [[ -n "$sec0" ]];then
      user_name=$(echo "Enter identity." | rofi -dmenu -p "Identity" -config ~/.config/waybar/scripts/rofi/networkmenu.rasi)
      if [[ -z "$user_name" ]];then
        exit 1
      fi
      password0=$(echo "Enter password of your identity (or leave empty)." | rofi -dmenu -p "Password" -config ~/.config/waybar/scripts/rofi/networkmenu.rasi)
      ...
      if [[ -n "$sec" ]];then #if network is secured, prompt for password
        password_=$(echo "Enter password of the newtork (or leave blank)." | rofi -dmenu -p "Password" -config ~/.config/waybar/scripts/rofi/networkmenu.rasi)
      fi
